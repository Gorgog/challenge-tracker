...
describe('ChallengesInsight — прогресс, потом честность сравнения', () => {
  ...
  it('пропуски в тяжёлые утра — сравнивать честно нельзя, и почему', () => {
    show()
    expect(within(card('Спорт')).getByText('Сравнить честно пока нельзя: «Спорт» чаще пропускался в дни, которые уже с утра были тяжёлыми (утро 3,0 против 7,0).')).toBeInTheDocument()
  })
  ...
  it('пропусков меньше 5 — «пока рано», и уже видно тяжёлые утра', () => {
    ...
    show()
    expect(
      within(card('Спорт')).getByText('Сравнивать пока рано: пропусков 4, нужно 5. И смотри: пропуски пришлись на утра хуже (3,0 против 7,0) — простое «в дни «Спорт» лучше» будет нечестным.'),
    ).toBeInTheDocument()
  })
