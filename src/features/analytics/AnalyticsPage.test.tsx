...
describe('AnalyticsPage — обзор: цель, фраза, график, «Что изменилось»', () => {
  ...
  it('нажатие на день открывает его: утро, вечер, теги и ночь словами', async () => {
    ...
    // подробности — под «Все оценки» (решение Georgy 25.09)
  ...
  it('с клавиатуры: ← → по дням, Enter и пробел открывают; сегодняшний вечер — «ещё не закрыт»', () => {
    ...
    // лента дня: у вечера — «ещё не закрыт» (решение Georgy 25.09)
    expect(within(screen.getByRole('dialog', { name: 'ср, 23 сентября' })).getByText('ещё не закрыт')).toBeInTheDocument()
  })
  ...
  it('шторка прошедшего дня: «Вечер не закрыт», отказ — «без срыва», «срыв» и «не записано»', async () => {
    ...
    expect(within(dialog).getByText('не закрыт')).toBeInTheDocument()
    // исходы челленджей словами — под «Все оценки» (решение Georgy 25.09)
    await user.click(within(screen.getByRole('dialog')).getByRole('button', { name: 'Все оценки' }))
    expect(within(within(dialog).getByRole('list', { name: 'Отметки дня' })).getByText('срыв')).toBeInTheDocument()
    await user.keyboard('{Escape}')
    await user.click(dayButton(/^сб, 19 сентября/))
    await user.click(within(screen.getByRole('dialog')).getByRole('button', { name: 'Все оценки' }))
    expect(within(within(screen.getByRole('dialog')).getByRole('list', { name: 'Отметки дня' })).getByText('без срыва')).toBeInTheDocument()
    await user.keyboard('{Escape}')
    ...
    expect(within(screen.getByRole('dialog')).queryByText('✗ Без сигарет')).toBeNull()
  })
